...
const images = {
  hero: "/images/ama/QC1.jpg",
  loom: "/images/ama/about-source.png",
  detail: "/images/products/Jeans2.jpg",
  fabric: "/images/products/Jeans3.jpg",
  production: "/images/ama/QC2.jpg",
  thread: "/images/products/Jeans4.jpg",
  warehouse: "/images/ama/QC4.jpg",
  portrait: "/images/products/Tops1.jpg",
};

const productImages = [
  ["/images/products/Jeans2.jpg", "Denim", "Washed denim"],
  ["/images/products/Jeans3.jpg", "Denim", "Everyday jeans"],
  ["/images/products/Jeans4.jpg", "Denim", "Texture & finish"],
  ["/images/products/shorts1.jpg", "Chino", "Utility chino"],
  ["/images/products/shorts3.jpg", "Chino", "Tailored cotton"],
  ["/images/products/shorts5.jpg", "Chino", "Pocket detail"],
  ["/images/products/shorts7.jpg", "Chino", "Washed khaki"],
  ["/images/products/shorts10.jpg", "Chino", "Cargo construction"],
  ["/images/products/Tops2.jpg", "Outerwear", "Zip detail"],
  ["/images/products/Tops3.jpg", "Outerwear", "Technical shell"],
  ["/images/products/Tops4.jpg", "Outerwear", "Embroidered finish"],
];
...
const galleryItems = [
  ...
  [productImages[0][0], productImages[0][1], productImages[0][2]],
  [productImages[3][0], productImages[3][1], productImages[3][2]],
  [productImages[4][0], productImages[4][1], productImages[4][2]],
  [productImages[8][0], productImages[8][1], productImages[8][2]],
  [productImages[2][0], productImages[2][1], productImages[2][2]],
  [productImages[5][0], productImages[5][1], productImages[5][2]],
  [productImages[6][0], productImages[6][1], productImages[6][2]],
  [productImages[10][0], productImages[10][1], productImages[10][2]],
];
...
function Reveal({ children, delay = 0, className = "" }: { children: React.ReactNode; delay?: number; className?: string }) {
  ...
  return (
    <motion.div ref={ref} className={className} initial={{ opacity: 0, y: 18 }} animate={visible ? { opacity: 1, y: 0 } : undefined} transition={{ duration: .45, delay, ease: "easeOut" }}>
      {children}
    </motion.div>
  );
}
...
function Header({ active, onNavigate }: { active: SectionKey; onNavigate: (id: SectionKey) => void }) {
  ...
  return <header className="fixed left-0 right-0 top-0 z-50 px-4 pt-4 sm:px-8 sm:pt-7"><div className="mx-auto flex max-w-[1440px] items-center justify-between rounded-full border border-[#f4f0e8]/10 bg-[#171817]/80 px-4 py-3 text-[#f4f0e8] shadow-2xl backdrop-blur-xl sm:px-6"><button onClick={() => onNavigate("home")} className="flex shrink-0 items-center gap-3"><LogoMark /><Mark dark /></button><nav className="hidden min-w-0 items-center gap-1 lg:flex">{navItems.map((item) => <button key={item.id} onClick={() => onNavigate(item.id)} className={`relative px-4 py-2 text-[11px] uppercase tracking-[.16em] transition-colors ${active === item.id ? "text-[#d09b50]" : "text-[#f4f0e8]/65 hover:text-[#f4f0e8]"}`}>{item.label}{active === item.id && <motion.span layoutId="activeNav" className="absolute bottom-0 left-4 right-4 h-px bg-[#d09b50]" />}</button>)}</nav><div className="hidden shrink-0 lg:block"><button onClick={() => onNavigate("contact")} className="rounded-full bg-[#d09b50] px-5 py-2.5 text-[11px] font-medium uppercase tracking-[.15em] text-[#171817] transition-transform hover:scale-[1.03]">Start a conversation</button></div><button className="lg:hidden" onClick={() => setOpen(!open)} aria-label="Toggle navigation">{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button></div>{open && <motion.nav initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mx-auto mt-2 flex max-w-[1440px] flex-col rounded-3xl border border-[#f4f0e8]/10 bg-[#171817] p-3 text-[#f4f0e8] lg:hidden">{navItems.map(item => <button key={item.id} onClick={() => { onNavigate(item.id); setOpen(false); }} className="rounded-2xl px-4 py-4 text-left text-xs uppercase tracking-[.16em] text-[#f4f0e8]/70 hover:bg-[#292a27] hover:text-[#d09b50]">{item.label}</button>)}</motion.nav>}</header>;
}
...
export default function MarketingDemo() {
  ...
  const submitContact = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/contact", { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(formData.entries())) });
      ...
      setContactError(error instanceof Error ? error.message : "Unable to send your enquiry.");
    }
  };
  ...
  return <div className="overflow-hidden bg-[#f3efe7] text-[#181a19]">
    ...
    <main>
      <section id="home" ref={heroRef} className="grain relative min-h-[820px] overflow-hidden bg-[#171817] text-[#f4f0e8] sm:min-h-screen">
        <div className="absolute inset-0 opacity-[.44]"><img src={images.hero} alt="Ali Murtaza Associates sewing floor" className="h-full w-full object-cover" /></div><div className="absolute inset-0 bg-gradient-to-b from-[#171817]/40 via-[#171817]/50 to-[#171817]" /><div className="absolute -right-48 top-32 hidden h-[520px] w-[520px] rounded-full border border-[#d09b50]/20 lg:block" /><div className="absolute -right-28 top-52 hidden h-[330px] w-[330px] rounded-full border border-[#d09b50]/15 lg:block" />
      ...
      <section id="gallery" className="px-6 py-28 sm:px-12 sm:py-40 lg:px-20"><div className="mx-auto max-w-[1440px]"><Reveal><div className="flex flex-wrap items-end justify-between gap-8"><div><p className="font-mono text-[10px] uppercase tracking-[.2em] text-[#b07d29]">05 / Selected output</p><h2 className="mt-5 text-5xl font-medium tracking-[-.07em] sm:text-7xl">A closer look.</h2></div><p className="max-w-xs text-sm leading-relaxed text-[#827c71]">Jeans, chinos, shorts and outerwear — a material-led look at the categories our teams bring to life.</p></div></Reveal><div className="mt-20 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{galleryItems.map(([src, category, title], i) => <Reveal key={`${title}-${i}`} delay={i * .025} className={`${i === 0 ? "sm:row-span-2" : ""}`}><button onClick={() => setSelectedGallery(i)} className="group relative block h-full min-h-[280px] w-full overflow-hidden bg-[#d8d0c1] text-left"><img src={src} alt={title} loading="lazy" className="absolute inset-0 h-full w-full object-cover grayscale transition-all duration-700 group-hover:scale-105 group-hover:grayscale-0" /><div className="absolute inset-0 bg-gradient-to-t from-[#171817]/75 via-transparent to-transparent opacity-70" /><div className="absolute bottom-5 left-5 right-5 flex items-end justify-between text-[#f4f0e8]"><div><span className="font-mono text-[9px] uppercase tracking-[.18em] text-[#d09b50]">{category}</span><p className="mt-1 text-lg tracking-[-.03em]">{title}</p></div><ArrowUpRightIcon /></div></button></Reveal>)}</div></div></section>
      ...
      <section id="contact" className="bg-[#171817] px-6 py-28 text-[#f4f0e8] sm:px-12 sm:py-40 lg:px-20"><div className="mx-auto grid max-w-[1440px] gap-16 lg:grid-cols-[.8fr_1.2fr]"><Reveal><p className="font-mono text-[10px] uppercase tracking-[.2em] text-[#d09b50]">07 / Make something</p><h2 className="mt-5 max-w-lg text-6xl font-medium leading-[.88] tracking-[-.08em]">Let’s make the next one count.</h2><p className="mt-8 max-w-sm text-sm leading-relaxed text-[#f4f0e8]/50">Tell us what you are building, where you are going and what good looks like. We will take it from there.</p><div className="mt-12 space-y-4 font-mono text-[11px] text-[#f4f0e8]/70"><a href="mailto:marketing@alimurtaza.com" className="block transition-colors hover:text-[#d09b50]">marketing@alimurtaza.com</a><span className="block">Karachi / Lahore / Pakistan</span></div></Reveal><Reveal delay={.15}><form onSubmit={submitContact} className="space-y-8"><div className="grid gap-8 sm:grid-cols-2"><label className="block"><span className="font-mono text-[10px] uppercase tracking-[.16em] text-[#d09b50]">Name</span><input name="name" required className="mt-3 w-full border-b border-[#f4f0e8]/25 bg-transparent py-3 text-lg outline-none transition-colors focus:border-[#d09b50]" placeholder="Your name" /></label><label className="block"><span className="font-mono text-[10px] uppercase tracking-[.16em] text-[#d09b50]">Company</span><input name="company" className="mt-3 w-full border-b border-[#f4f0e8]/25 bg-transparent py-3 text-lg outline-none transition-colors focus:border-[#d09b50]" placeholder="Your company" /></label></div><label className="block"><span className="font-mono text-[10px] uppercase tracking-[.16em] text-[#d09b50]">Email</span><input name="email" type="email" required className="mt-3 w-full border-b border-[#f4f0e8]/25 bg-transparent py-3 text-lg outline-none transition-colors focus:border-[#d09b50]" placeholder="you@company.com" /></label><label className="block"><span className="font-mono text-[10px] uppercase tracking-[.16em] text-[#d09b50]">Project brief</span><textarea name="message" required rows={3} className="mt-3 w-full resize-none border-b border-[#f4f0e8]/25 bg-transparent py-3 text-lg outline-none transition-colors focus:border-[#d09b50]" placeholder="What are we making together?" /></label><button disabled={contactStatus === "sending"} className="group inline-flex items-center gap-3 rounded-full bg-[#d09b50] px-6 py-3 text-[11px] font-medium uppercase tracking-[.15em] text-[#171817] transition-transform hover:scale-[1.03] disabled:cursor-wait disabled:opacity-70">{contactStatus === "sending" ? "Sending…" : contactStatus === "sent" ? "Enquiry sent" : "Send enquiry"} {contactStatus === "sent" ? <Check className="h-4 w-4" /> : <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />}</button>{contactStatus === "error" && <p role="alert" className="text-sm text-[#e6a09a]">{contactError}</p>}{contactStatus === "sent" && <p role="status" className="text-sm text-[#d0c8b7]">Thanks — the AMA team has received your enquiry and will be in touch.</p>}</form></Reveal></div></section>
    </main>
    <footer className="bg-[#171817] px-6 pb-8 text-[#f4f0e8] sm:px-12 lg:px-20"><div className="mx-auto flex max-w-[1440px] flex-col justify-between gap-8 border-t border-[#f4f0e8]/15 pt-8 sm:flex-row sm:items-end"><div><Mark dark /><p className="mt-4 font-mono text-[10px] uppercase tracking-[.14em] text-[#f4f0e8]/35">Woven apparel manufacturing<br />Karachi · Lahore · Since 1986</p></div><div className="flex flex-wrap gap-x-5 gap-y-3 font-mono text-[10px] uppercase tracking-[.15em] text-[#f4f0e8]/45 sm:justify-end"><a href="https://www.alimurtaza.com/" target="_blank" rel="noreferrer" className="hover:text-[#d09b50]">Original site</a><span>© {new Date().getFullYear()} AMA</span></div></div></footer>
